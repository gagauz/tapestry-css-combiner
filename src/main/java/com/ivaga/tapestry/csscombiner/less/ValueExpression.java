...
class ValueExpression extends Expression {
	...
	/**
	 * Create a value expression as parameter for a mixin which not change it
	...
	public static ValueExpression eval(CssFormatter formatter, Expression expr) {
		expr = expr.unpack(formatter); // unpack to increase the chance to find
										// a ValueExpression
			...
			if (str.startsWith("#")) {
				...
				if (rgb == -1) {
					if (str.equals("transparent")) {
						...
						if (number == null) {
							if (str.startsWith("+")) { // DecimalFormat can not
														// parse an option plus
														// sign
								pos.setIndex(1);
