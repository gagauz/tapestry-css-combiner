...
class LessLookAheadReader extends LessObject implements Closeable {
	 ...
	 * @param fileName
	 *            the filename of the less file or null if a String is parsed.
	 * @param isReference
	 *            true, if the less file is imported as reference
	 */
	...
	/**
	 * Get the next parse type. This can be -1, ';', '{' or '}'. It copy the
	 * input until this marker in the look ahead cache.
	 * 
		...
		try {
			for (;;) {
				...
				if (ch < 0) {
					for (int i = 0; i < cache.length(); i++) {
						if (!Character.isWhitespace(cache.charAt(i))) {
							return ';'; // a not terminated line is like a lime
										// with semicolon
						}
					}
					return -1;
				...
				case '/':
					if (isSlash) {
						if (parenthesis > 0 && cache.indexOf("url") > 0) {
							break; // url function with unquoted url like
									// url(http://xyz)
						}
				...
				case '*':
					...
					break;
				case '{':
					if (cache.length() > 1 && cache.charAt(cache.length() - 2) == '@') { // @{
																							// -->
																							// a
																							// inline
																							// variable
	...
	/**
	 * If the next data which are already in the cache are a mixin parameter or
	 * part of a selector name. This is call after a left parenthesis. Samples
	 * for selectors:
	 * <ul>
	 ...
	 * 
	 * @param isBlock
	 *            selector of a block or a semicolon line
		...
		for (int i = cachePos; i < cache.length(); i++) {
			...
			case '.':
				if (!isFirst) {
					...
					if (Character.isDigit(cache.charAt(i + 1))) { // Number with
																	// a
																	// starting
																	// point
						return true;
