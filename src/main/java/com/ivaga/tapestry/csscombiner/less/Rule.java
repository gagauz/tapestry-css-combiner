...
class Rule extends LessObject implements Formattable, FormattableContainer {
	 ...
	 * @param params
	 *            the parameter if the rule is a mixin.
	 * @param guard
	 *            a guard condition for the mixin or CSS rule
	 */
	Rule(LessObject obj, FormattableContainer parent, String selectors, Operation params, Expression guard) {
		super(obj);
		this.parent = parent;
	 ...
	 * 
	 * @param mainSelector
	 *            optional selectors of the calling rule if this is a mixin.
	 * @param formatter
	 *            current formatter
	 */
	void appendTo(String[] mainSelector, CssFormatter formatter) {
		try {
			...
			for (int s = 0; s < sel.length; s++) {
				...
				if (selector != str) {
					if (sel == selectors) {
						sel = sel.clone(); // we does not want change the
											// declaration of this selectors
					}
					sel[s] = str;
				}
			}
	...
	private void bubbling(String[] mediaSelector, String[] blockSelector, CssFormatter formatter) {
		if (properties.size() > 0) {
			...
			if (media.startsWith("@media") || media.startsWith("@supports") || media.startsWith("@document")) {
				...
				// non-conditional directives for example @font-face or
				// @keyframes
				CssFormatter block = formatter.startBlock(mediaSelector);
				appendPropertiesTo(block);
				block.endBlock();
	...
	/**
	 * Get the mixin parameters as map if the given param values match to this
	 * rule.
	 * 
	 * @param formatter
	 *            current formatter
	 * @param paramValues
	 *            the values of the caller
	 * @return null, if empty list match; NO_MATCH, if the values not match to
	 *         the params of this list, Or the map with the parameters
	 */
	private Map<String, Expression> getMixinParams(CssFormatter formatter, List<Expression> paramValues) {
		if ((params == null && paramValues == null) || (paramValues == null && params.size() == 0)
				|| (params == null && paramValues.size() == 0)) {
			return null;
		}
		...
		try {
			...
			for (int i = 0; i < paramsCount; i++) {
				...
				if (paramType == Operation.class && ((Operation) param).getOperator() == ':'
						&& ((Operation) param).getOperands().size() == 2) {
			...
			for (int i = 0; i < paramsCount; i++) {
				...
				if (valueType == Operation.class && ((Operation) value).getOperator() == ':'
						&& ((Operation) value).getOperands().size() == 2) {
					...
					} else if (paramType == Operation.class && ((Operation) param).getOperator() == ':'
							&& ((Operation) param).getOperands().size() == 2) {
						ArrayList<Expression> keyValue = ((Operation) param).getOperands();
	 ...
	 *            calling parameters
	 * @param isDefault
	 *            the value of the keyword "default" in guard.
	...
	/**
	 * If this rule is a CSS rule (not a mixin declaration) and if an guard
	 * exists if it true.
	 * 
	 * @param formatter
	 *            the CCS target
	...
	/**
	 * If there is only a special selector "&" and also all subrules have only
