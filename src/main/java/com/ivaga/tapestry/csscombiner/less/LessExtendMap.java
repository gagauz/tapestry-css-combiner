...
class LessExtendMap {
	...
	// use a LinkedHashSet as cache to remove duplicates and hold the original
	// order
	 ...
	 *            the extend
	 * @param mainSelector
	 *            the selectors in which the extend is placed.
	 */
	...
	/**
	 * Add to the given selectors all possible extends and return the resulting
	 * selectors.
	 * 
	 * @param selectors
	 *            current selectors
	 * @param isReference
	 *            if the current rule is in a less file which was import with
	 *            "reference" keyword
	 * @return the selectors concatenate with extends or the original if there
	 *         are no etends.
	 */
	...
	/**
	 * Add to the given selector all possible extends to the internal
	 * selectorList. This method is call recursive.
	 * 
	 * @param selector
	 *            current selector
	 * @param isReference
	 ...
	 *            "reference" keyword
	 * @param allSelector
	 *            selector that should match with the "all" keyword. By default
	 *            is this equals of the selector. Only on recursion this is only
	 *            the replaced part.
	 */
		...
		if (list != null) {
			for (String[] lessExtend : list) {
				for (String sel : lessExtend) {
					...
					if (needRecursion) { // only if there are new entries then
											// we need to try a recursion, else
