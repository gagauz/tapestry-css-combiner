...
class LessParser implements FormattableContainer {
	...
	/**
	 * A StringBuilder which can reused inside one method. Do not call another
	 * method that also use it.
	 */
	...
	/**
	 * If there are some imports with variables then this will parse after a
	 * formatter if available.
	 *
	 * @param formatter
	 *            the formatter to evaluate variables
	 */
	 ...
	 *
	 * @param currentRule
	 *            the current container. This can be the root or a rule.
	 */
	...
	/**
	 * Parse an expression which ends with a semicolon. This can be a property
	 * assignment, variable assignment, a directive or other.
	 *
	 * @param currentRule
	 *            the current container. This can be the root or a rule.
	 */
	private void parseSemicolon(FormattableContainer currentRule) {
		...
		LOOP: for (;;) {
			...
			case '@':
				ch = read();
				if (ch == '{') { // @{ --> a inline variable and not a variable
									// declaration
					builder.append('@');
					builder.append(ch);
					do {
	...
	void parseBlock(FormattableContainer currentRule) {
		...
		for (;;) {
			...
			case '{':
				...
				if (selector.endsWith("@")) { // selector with variable name
												// like abc@{foo}abc
	 ...
	 * @param currentRule
	 *            parent container which contains the variable
	 */
	private void variable(HashMap<String, Expression> variables, FormattableContainer currentRule) {
		...
		LOOP: for (;;) {
			...
			case ';':
				...
				currentRule.add(new CssAtRule(reader, name + ';')); // directives
																	// like
																	// @charset
																	// "UTF-8";
				return;
	 ...
	 *
	 * @param currentRule
	 *            current container. This can be the root or a rule.
	 * @param name
	 *            the content of the directive like a file name and keywords
	 * @throws LessException
	 *             if any parsing error occurred
	 */
	private void importFile(FormattableContainer currentRule, final String name) {
		...
		Object[] old = { reader, baseURL, relativeURL }; // store on the heap to
															// reduce the stack
															// size
		try {
			int i;
			...
			if (filename.contains("@{")) { // filename with variable name, we
											// need to parse later
				if (currentRule != this) {
					// import is inside of a mixin and will be process if the
					// mixin will be process
			...
			if (!isLess && (isCss || filename.endsWith("css"))) {
				// filenames ends with "css" will not be inline else a CSS
				// @import directive is written
				currentRule.add(new CssAtRule(reader, "@import " + name + ';'));
				return;
			}
			baseURL = baseURL == null ? new URL(filename) : new URL(baseURL, filename);
			if (!isLess && baseURL.getPath().endsWith("css")) {
				// URL path ends with "css" will not be inline else a CSS
				// @import directive is written
				currentRule.add(new CssAtRule(reader, "@import " + name + ';'));
				return;
			}
	 ...
	 * @param params
	 *            the parameters if it is a mixin.
	 * @param guard
	...
	/*
	 * private void rule( Rule rule ) { StringBuilder builder = cachesBuilder;
	 * Operation expr = null; LOOP: for( ;; ) { char ch = read(); switch( ch ) {
	 * case ':': switch( firstNonWhitespace( builder ) ) { case '&': //
	 * pseudo-selectors case '.': // Mixin case '#': // Mixin builder.append( ch
	 * ); continue LOOP; } String name = trim( builder ); Expression value =
	 * parseExpression( (char)0 ); ch = read(); switch( ch ) { case '}': //last
	 * line in a block does not need a semicolon back( ch ); break; case ';':
	 * break; default: throw createException( "Unrecognized input: '" + ch + "'"
	 * ); } rule.add( new RuleProperty( name, value ) ); break; case ';':
	 * rule.add( new Mixin( trim( builder ), expr, mixins ) ); expr = null;
	 * break; case '}': throwUnrecognizedInputIfAny( builder, ch ); return; case
	 * '{': // nested rules name = trim( builder ); rule.add( rule( name, expr,
	 * null ) ); expr = null; break; case '(': name = trim( builder ); expr =
	 * parseParameterList(); builder.append( name ); break; case '/': String
	 * comment = comment(); if( comment != null && isWhitespace( builder ) ) {
	 * rule.add( new Comment( comment ) ); } break; case '@':
	 * throwUnrecognizedInputIfAny( builder, ch ); variable(
	 * rule.getVariables(), rule ); break; default: builder.append( ch ); } } }
	 */
	 ...
	 *
	 * @param leftOperator
	 *            the operator on the left side or 0 if this is the first
	 *            expression.
	 * @return the expression or null if it an empty function parameter list
	 *         like ()
	 */
	...
	/**
	 * Parse the parameter of an "url" function which has some curious
	 * fallbacks.
	 *
	 ...
	 *
	 * @param str
	 *            the expression like a number, color, variable, string, etc
	 ...
	 *
	 * @param container
	 *            optional container for the parsed comments
	 *
	 * @return return true, if a comment was parsed, false if the slash must be
	 *         parse anywhere else
	 */
	...
	/**
	 * Check if the current content is an identifier or an expression. This is
	 * call to decide if a minus is part of this identifier or an operation.
	 * Identifier are:
	 * <li>variables
	 * <li>mixins
	 * <li>selectors</li>
	...
	/**
	 * The first character in the builder that is not a whitespace. Else there
	...
	/**
	 * Throw an unrecognized input exception if there content in the
	 * StringBuilder.
	 *
	 * @param builder
	 *            the StringBuilder
