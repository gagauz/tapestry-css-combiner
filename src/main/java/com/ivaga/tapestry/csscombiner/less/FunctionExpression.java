...
class FunctionExpression extends Expression {
	 ...
	 */
	FunctionExpression(LessObject obj, String name, Operation parameters) {
		super(obj, name);
	...
	/**
	 * Write the function without change. We does not know it. It can/must be a
	 * CSS function.
	 *
	 * @param formatter
	 *            the formatter
		...
		try {
			...
			case "min":
				...
				for (int i = 1; i < parameters.size(); i++) {
					...
					doubleValue = Math.min(doubleValue,
							param.doubleValue(formatter) / Operation.unitFactor(unit, param.unit(formatter), true));
				}
				return;
			case "max":
				type = NUMBER;
				...
				for (int i = 1; i < parameters.size(); i++) {
					param = parameters.get(i);
					doubleValue = Math.max(doubleValue,
							param.doubleValue(formatter) / Operation.unitFactor(unit, param.unit(formatter), true));
				}
				return;
			case "sqrt":
	...
	/**
	 * Implementation of the escape function:
	 ...
	 *             if the parameter with the index does not exists
	 */
	...
	/**
	 * Get the idx parameter from the parameter list as color value. And set the
	 * type variable.
	 *
	 * @param idx
	 *            the index starting with 0
