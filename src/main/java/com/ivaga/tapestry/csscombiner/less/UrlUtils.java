...
class UrlUtils {
	...
	static void svgGradient(CssFormatter formatter, List<Expression> parameters) {
		if (parameters.size() < 3) {
			throw new LessException(
					"error evaluating function `svg-gradient expects direction, start_color [start_position], [color position,]..., end_color ");
		}
		...
		default:
			throw new LessException(
					"error evaluating function `svg-gradient`: svg-gradient direction must be 'to bottom', 'to right', 'to bottom right', 'to top right' or 'ellipse at center'");
		}
		...
		builder.append('<').append(gradientType).append("Gradient id=\"gradient\" gradientUnits=\"userSpaceOnUse\" ")
				.append(gradientDirection).append('>');
		...
		builder.append("</").append(gradientType).append("Gradient><rect ").append(rectangleDimension)
				.append(" fill=\"url(#gradient)\" /></svg>");
	...
	/**
	 * Get the color value of the expression or fire an exception if not a
	 * color.
	 *
	 * @param param
	 ...
	 *             if the expression is not a color value
	 */
	 ...
	 *            current formatter
	 * @param relativeURL
	 *            relative URL of the less script. Is used as base URL
	 * @param urlString
	 *            the url parameter of the function
	 * @param type
	 *            the mime type
	 * @throws IOException
	 *             If any I/O errors occur on reading the content
	 */
	 ...
	 *            the bytes
	 * @param urlStr
	 *            used if mime type is null to detect the mime type
	 * @param type
	 *            the mime type
	 */
