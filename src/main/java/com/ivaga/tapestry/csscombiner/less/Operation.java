...
class Operation extends Expression {
	 ...
	 * 
	 * @param group
	 *            the unit group like length, duration or angles. Units of one
	 *            group can convert in another.
	 * @param unit
	 *            the unit name
	 * @param factor
	 *            the convert factor
	...
	/**
	 * Create a new instance with an empty parameter list and a comma as
	 * operation.
	 * 
	 * @param obj
	 *            another LessObject with parse position.
	...
	public void appendTo(CssFormatter formatter) {
		...
		case LIST:
			...
			case '~':
				...
				formatter.append(builder.toString()); // new ValueExpression(
														// this,
														// builder.toString()
														// ).appendTo( formatter
														// );
	 ...
	 *            right unit
	 * @param fail
	 *            true, should be fail if units incompatible; false, return 1 is
	 *            incompatible
	 * @return the factor between the 2 units.
	 * @throws LessException
	 *             if unit are incompatible and fail is true
	 */
	...
	/**
	 * Calculate a color on left with a number on the right side. The
	 * calculation occur for every color channel.
	 * 
	 * @param color
	 *            the color
	 * @param right
	...
	/**
	 * Calculate a number on left with a color on the right side. The
	 * calculation occur for every color channel.
	 * 
	 * @param left
	 *            the left
	...
	public boolean booleanValue(CssFormatter formatter) {
		...
		case '≤':
			...
			switch (type) {
			case STRING: {
				// need to differ between keyword without quotes and strings
				// with quotes. The type of quote is ignored
	...
	/**
	 * Calculate the unit if there are different units. It use the numerator and
	 * denominator count.
	 * 
	 * @param formatter
	 *            the CCS target
	...
	/**
	 * The level of the operation. This is used to detect if the left or right
	 * operator has the higher level.
	 * 
	 * @param operator
	...
	/**
	 * Helper class to count the resulting unit of an operation with different
	 * units
	 */
